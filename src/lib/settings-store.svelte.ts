...
import {
	getDefaultTitlebarLayout,
	type TitlebarStyleSetting
} from './platform';
...
class SettingsStore {
    ...
    playbackDelay = $state(500);
    titlebarLayout = $state<string[]>(getDefaultTitlebarLayout());
    titlebarStyle = $state<TitlebarStyleSetting>('auto');
    volume = $state(1);
    ...
    filenameQuery = $state('');
    recentCollections = $state<string[]>([]);
    collectionUiByPath = $state<Record<string, CollectionUiState>>({});
        ...
        if (typeof window !== 'undefined') {
            ...
            if (savedSettings) {
                try {
                    ...
                    if (parsed.titlebarStyle === 'macos' || parsed.titlebarStyle === 'windows' || parsed.titlebarStyle === 'linux') {
                    ...
                    if (parsed.titlebarLayout) {
                        if (!Array.isArray(parsed.titlebarLayout)) {
                            ...
                            this.titlebarLayout = parsed.titlebarLayout.filter(
                                (id: string) => id !== 'loop'
                            );
                        }
                    }
            ...
            $effect(() => {
                if (typeof window !== 'undefined') {
                    localStorage.setItem('settings', JSON.stringify({
                        ...
                        selectedFormats: $state.snapshot(this.selectedFormats),
                        selectedTags: $state.snapshot(this.selectedTags),
                        filenameQuery: this.filenameQuery,
                        recentCollections: $state.snapshot(this.recentCollections),
                        collectionUiByPath: $state.snapshot(this.collectionUiByPath)
                    }));
                }
            });
        });
    ...
    addRecentCollection(path: string) {
        ...
        this.recentCollections = [...this.recentCollections, normalized].slice(
            -RECENT_COLLECTIONS_MAX
        );
    }
